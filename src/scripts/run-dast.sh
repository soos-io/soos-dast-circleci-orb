...
PARAMS="--clientId ${!SOOS_CLIENT_ID_VAR_NAME} --apiKey ${!SOOS_API_KEY_VAR_NAME} --projectName ${SOOS_PROJECT_NAME} --scanMode ${SOOS_SCAN_MODE} --apiURL ${SOOS_API_BASE_URL} --integrationName='CircleCI' --integrationType='Plugin' --commitHash ${CIRCLE_SHA1} --branchName ${CIRCLE_BRANCH}"
